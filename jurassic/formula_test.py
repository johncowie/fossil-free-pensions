...
def test_pool_match_formula():
    expected = '=ARRAYFORMULA(IFS(A:A="Company 1", "yes", A:A="Company 3", "yes", TRUE, "no"))'
    ...
    assert formula.pooled_match("A:A", []) == 'no'
    assert formula.pooled_match("A:A", [{'Is Pooled? (Y/N)':'N'}]) == 'no'
